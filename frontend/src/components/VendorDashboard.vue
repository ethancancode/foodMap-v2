...
async function adjustPortion(item, delta) {
  ...
  try {
    ...
    if (idx !== -1) {
      listings.value[idx] = { ...listings.value[idx], ...updated, quantity: newQty, isAvailable: newQty > 0 }
    }
    emit('action', {
      action: 'toast',
...
async function toggleSoldOut(item) {
  ...
  try {
    ...
    const idx = listings.value.findIndex((f) => f._id === (updated?._id || item._id))
    if (idx !== -1) {
      listings.value[idx] = { ...listings.value[idx], ...updated, isAvailable: newAvailable, quantity: newQty }
    }
    emit('action', {
      action: 'toast',
