...
onMounted(async () => {
  try {
    const [foodsRes, vendorRes] = await Promise.all([
      foodApi.getFoods(),
    ...
    items.value = allFoods.map((f) => ({
      ...
      desc: f.description,
      image: f.image || 'https://lh3.googleusercontent.com/aida-public/AB6AXuB5tj44PBk_pXA_-WXZGHdA-t1aeLtsjItWwoODao564te6kYMnEP0nty7ZmOM1uGDAyCKeo4ZYgD29FWPxya4vFyDPBKuIYhfbZ2r_4nqxWX2NWHuZSb-U__HojUGlV7Qry_kdbWUTrcyLsEX02meeaz_G9Og5CwwN55pjxKG7NpNoYWSjq69eI9DSj9SGrl1IVowEZ0jKsCnvaymZkSjT65zjQQSkEjmH1Cu6jClqXYIOkburuor5'
    }))
  } catch (e) {
...
function selectFood(item) {
  emit('navigate', 'food_details', {
    food: {
      ...
      name: item.name,
      price: item.price,
      vendor: vendorProfile.value?.businessName || props.user?.name || "Anjali's Kitchen",
      distance: '420m away',
      portions: parseInt(item.portions) || 6,
      time: item.time,
      image: item.image,
      desc: item.desc
    }
